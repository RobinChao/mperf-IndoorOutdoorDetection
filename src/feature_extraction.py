...
def time_of_day(t, tz) -> str:
    ...
    if (local_t.hour >= MORNING_START and local_t.hour < MORNING_END):
        return 'Morning'
    elif local_t.hour >= NOON_START and local_t.hour < NOON_END:
        return 'Noon'
    elif local_t.hour >= AFTERNOON_START and local_t.hour < AFTERNOON_END:
        return 'Afternoon'
    elif local_t.hour >= EVENING_START and local_t.hour < EVENING_END:
        return 'Evening'
    else:
        return 'Night'


def data_in_tw(df, start, end):
...
def extract_features(streams, start_t, end_t, t_win_s) -> List:
    ...
    while t2 < end_t:
        ...
        feature = extract_features_from_tw(data, t1)
